<template>
  <UiCard>
    <h3 class="text-lg font-semibold text-gray-900 mb-4">Paketlenecek Siparişler</h3>
    ...
    <div v-else-if="orders && orders.length > 0" class="space-y-3">
      ...
      >
        ...
        <UiButton
          size="sm"
          :disabled="!canUpdate"
          :title="!canUpdate ? 'Paketleme yetkiniz yok' : ''"
        >
          Paketle
        </UiButton>
      </div>
    ...
    <div v-else class="flex flex-col items-center justify-center py-12 text-gray-500">
      ...
      <p class="text-sm text-gray-600 text-center max-w-sm">
        All orders are packed and ready for the next step.
      </p>
    </div>
  </UiCard>
</template>
...
const orders = computed(() => queuesData.value?.toPack ?? [])
